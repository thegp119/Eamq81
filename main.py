...
print(f"🔴 RAW BOT_TOKEN length: {len(BOT_TOKEN)}")
print(f"🔴 RAW BOT_TOKEN bytes: {BOT_TOKEN.encode('utf-8')}")
print(f"🔴 RAW BOT_TOKEN repr: {repr(BOT_TOKEN)}")

# Show exact hex of ALL characters
if BOT_TOKEN:
    all_hex = ' '.join(f'{ord(c):02x}' for c in BOT_TOKEN)
    print(f"🔍 BOT_TOKEN ALL chars (hex): {all_hex}")

# Try multiple cleaning methods
BOT_TOKEN_cleaned = BOT_TOKEN.strip()
print(f"After .strip(): {len(BOT_TOKEN_cleaned)}")

BOT_TOKEN_cleaned = ''.join(BOT_TOKEN_cleaned.split())
print(f"After .split() and join: {len(BOT_TOKEN_cleaned)}")

BOT_TOKEN = BOT_TOKEN_cleaned

# Clean STRIPE key
STRIPE_API_KEY = ''.join(STRIPE_API_KEY.split())
