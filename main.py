...
async def parse_ubereats_group_link(link: str) -> dict:
    """Parse Uber Eats group order link to extract order details using Playwright"""
    try:
        print(f"Attempting to parse Uber Eats link with Playwright: {link}")
        async with async_playwright() as p:
            ...
            try:
                ...
                # Try to extract restaurant name from page text or title
                try:
                    restaurant = await page.evaluate("document.title")
                    if restaurant:
                        restaurant = restaurant.split("|")[0].strip()
                    if not restaurant or restaurant == "Unknown Restaurant":
                        # Try to find in page content
                        heading = await page.query_selector("h1, [data-test='restaurant-name']")
                        if heading:
                            restaurant = await heading.inner_text()
                except:
                    pass
                ...
                try:
                    data = await page.evaluate("""
                        () => {
                            let subtotal = 0, tax = 0, delivery = 0;
                            
                            // Try to find price elements
                            const priceElements = document.querySelectorAll('[data-test*="price"], [class*="price"], [class*="total"]');
                            const prices = Array.from(priceElements).map(el => {
                                const text = el.innerText || el.textContent;
                                return text.replace(/[^0-9.]/g, '');
                            }).filter(p => p.length > 0);
                            
                            // Look for specific patterns in page text
                            const pageText = document.body.innerText;
                            const subtotalMatch = pageText.match(/Subtotal.*?\\$([\d.]+)/i);
                            const taxMatch = pageText.match(/Tax.*?\\$([\d.]+)/i);
                            const deliveryMatch = pageText.match(/Delivery.*?\\$([\d.]+)/i);
                            
                            if (subtotalMatch) subtotal = parseFloat(subtotalMatch[1]);
                            if (taxMatch) tax = parseFloat(taxMatch[1]);
                            if (deliveryMatch) delivery = parseFloat(deliveryMatch[1]);
                            
                            // If no match, use largest price found as subtotal
                            if (subtotal === 0 && prices.length > 0) {
                                const nums = prices.map(p => parseFloat(p)).filter(n => n > 10 && n < 500);
                                subtotal = Math.max(...nums, 0);
                            }
                            
                            return {subtotal, tax, delivery, prices: prices.slice(0, 10)};
                        }
                    """)
                    ...
                    print(f"Extracted - Subtotal: ${subtotal}, Tax: ${tax}, Delivery: ${delivery_fee}")
                    print(f"Found prices: {data.get('prices', [])}")
                except Exception as e:
                    print(f"Error extracting prices via JavaScript: {e}")
                
                # Get the full page HTML as backup
