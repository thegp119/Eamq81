...
# Show exact hex of first and last 5 chars to debug whitespace
if BOT_TOKEN:
    first_5_hex = ' '.join(f'{ord(c):02x}' for c in BOT_TOKEN[:5])
    last_5_hex = ' '.join(f'{ord(c):02x}' for c in BOT_TOKEN[-5:])
    print(f"🔍 BOT_TOKEN first 5 chars (hex): {first_5_hex}")
    print(f"🔍 BOT_TOKEN last 5 chars (hex): {last_5_hex}")

# Strip ALL whitespace types including unicode spaces
BOT_TOKEN = ''.join(BOT_TOKEN.split())
STRIPE_API_KEY = ''.join(STRIPE_API_KEY.split())

# Debug: Log what we found
print(f"📋 BOT_TOKEN after cleaning: length {len(BOT_TOKEN)}")
print(f"📋 STRIPE_API_KEY after cleaning: length {len(STRIPE_API_KEY)}")
