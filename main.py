        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(link, timeout=aiohttp.ClientTimeout(total=10), ssl=False, allow_redirects=True) as resp:
                ...
                if resp.status == 200:
                    ...
                    restaurant_match = re.search(r'"restaurantName":"([^"]+)"', html)
                    restaurant = restaurant_match.group(1) if restaurant_match else "Unknown Restaurant"
                    print(f"Restaurant detected: {restaurant}")
                    
                    # Try to extract items and prices from JSON data
                    items = []
                    subtotal = 0.0
                    tax = 0.0
                    delivery_fee = 0.0
                    ...
                    # Look for cart items
                    items_match = re.search(r'"items":\s*\[(.*?)\]', html)
                    if items_match:
                        items_str = items_match.group(1)
                        # Extract individual items with quantities and prices
                        item_pattern = r'"title":"([^"]+)".*?"quantity":(\d+).*?"price":(\d+)'
                        for match in re.finditer(item_pattern, items_str):
                            item_name = match.group(1)
                            qty = int(match.group(2))
                            price_cents = int(match.group(3))
                            price_dollars = price_cents / 100
                            items.append({"name": item_name, "quantity": qty, "price": price_dollars})
                            subtotal += price_dollars * qty
                    
                    # Try to extract totals
                    subtotal_match = re.search(r'"subTotal":(\d+)', html)
                    if subtotal_match:
                        subtotal = int(subtotal_match.group(1)) / 100
                    
                    tax_match = re.search(r'"tax":(\d+)', html)
                    if tax_match:
                        tax = int(tax_match.group(1)) / 100
                    
                    delivery_match = re.search(r'"deliveryFee":(\d+)', html)
                    if delivery_match:
                        delivery_fee = int(delivery_match.group(1)) / 100
                    
                    print(f"Pricing - Subtotal: ${subtotal}, Tax: ${tax}, Delivery: ${delivery_fee}")
