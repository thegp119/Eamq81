        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(link, timeout=aiohttp.ClientTimeout(total=10), ssl=False, allow_redirects=True) as resp:
                ...
                if resp.status == 200:
                    ...
                    try:
                        with open("/tmp/ubereats_html.html", "w", encoding="utf-8") as f:
                            f.write(html)
                        print("Saved HTML to /tmp/ubereats_html.html for inspection")
                    ...
                    json_match = re.search(r'<script[^>]*id="__INITIAL_STATE__"[^>]*>(.*?)</script>', html, re.DOTALL)
                    if json_match:
                        try:
                            ...
                            # Extract restaurant info from nested JSON
                            if 'cart' in data and 'restaurant' in data['cart']:
                                restaurant = data['cart']['restaurant'].get('name', 'Unknown Restaurant')
                                print(f"Restaurant from JSON: {restaurant}")
                            ...
                            if 'cart' in data and 'items' in data['cart']:
                                for item in data['cart']['items']:
                                    ...
                                    items.append({"name": item_name, "quantity": qty, "price": price})
                                    subtotal += price * qty
                            ...
                            if 'cart' in data:
                                ...
                                tax = cart.get('tax', 0) / 100
                                delivery_fee = cart.get('deliveryFee', 0) / 100
                                print(f"Cart totals - Subtotal: ${subtotal}, Tax: ${tax}, Delivery: ${delivery_fee}")
                        except Exception as e:
                            print(f"Error parsing JSON: {e}")
                    
                    # If JSON parsing didn't work, try regex patterns as fallback
                    if not items and not subtotal:
                        print("JSON parsing failed, trying regex patterns...")
                        
                        # Try alternative patterns for restaurant name
                        rest_patterns = [
                            r'"restaurant":\s*\{[^}]*"name":"([^"]+)"',
                            r'<h1[^>]*>([^<]+)</h1>',
                            r'"restaurantName":"([^"]+)"'
                        ...
                        for pattern in rest_patterns:
                            ...
                            if match:
                                ...
                                print(f"Restaurant found with regex: {restaurant}")
                                break
                        
                        # Look for price patterns
                        price_patterns = [
                            (r'"subtotal":"?(\d+)"?', lambda x: int(x) / 100),
                            (r'"total":"?(\d+)"?', lambda x: int(x) / 100),
                            (r'\$(\d+\.\d+)', lambda x: float(x))
                        ]
                        for pattern, converter in price_patterns:
                            match = re.search(pattern, html)
                            if match and not subtotal:
                                try:
                                    subtotal = converter(match.group(1))
                                    print(f"Subtotal found: ${subtotal}")
                                    break
                                except:
                                    pass
