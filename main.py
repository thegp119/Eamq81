        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(link, timeout=aiohttp.ClientTimeout(total=10), ssl=False, allow_redirects=True) as resp:
                ...
                if resp.status == 200:
                    ...
                    # Extract all prices from the HTML that look like monetary values
                    price_matches = re.findall(r'"(?:subtotal|total|amount|deliveryFee|fee|price|taxAmount)"[\'"]?\s*:\s*[\'"]?(\d+(?:\.\d{2})?)[\'"]?', html)
                    print(f"Found {len(price_matches)} price values in HTML")
                    
                    # Look for restaurant name
                    rest_patterns = [
                        r'"restaurantName"\s*:\s*"([^"]+)"',
                        r'"restaurant"\s*:\s*\{"name"\s*:\s*"([^"]+)"',
                        r'"name"\s*:\s*"([^"]+)"\s*,\s*"[^"]*"\s*:\s*"[^"]*"\s*,\s*"[^"]*\d+"',
                    ]
                    ...
                    # Try to extract prices from common patterns
                    subtotal_match = re.search(r'"subtotal"\s*:\s*(\d+)', html)
                    if subtotal_match:
                    ...
                    tax_match = re.search(r'"tax"\s*:\s*(\d+)', html)
                    if tax_match:
                    ...
                    delivery_match = re.search(r'"deliveryFee"\s*:\s*(\d+)', html)
                    if not delivery_match:
                        delivery_match = re.search(r'"fee"\s*:\s*(\d+)', html)
                    if delivery_match:
                        delivery_fee = int(delivery_match.group(1))
                        if delivery_fee > 100:
                            delivery_fee = delivery_fee / 100
                        print(f"Delivery fee found: ${delivery_fee}")
                    
                    # If subtotal is still 0, try finding the largest reasonable price in the HTML
                    if subtotal == 0 and price_matches:
                        try:
                            prices = []
                            for p in price_matches:
                                val = float(p)
                                if 10 < val < 500:  # Reasonable order price range
                                    prices.append(val)
                            if prices:
                                subtotal = max(prices)
                                print(f"Using largest price as subtotal: ${subtotal}")
                        except:
                            pass
